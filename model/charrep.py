...
class CharRep(nn.Module):
    ...
    def __init__(self, config_dic, char_vocab_dim, dropout_rate: float):
        super().__init__()
        self.gpu = config_dic.get("gpu")
        self.char_emb_dim = config_dic.get("char_emb_dim")
        self.char_hidden_dim = config_dic.get("char_hidden_dim")# bidirectional
    ...
    def get_last_hiddens(self, char_features):
        ...
        char_embeds = self.dropout(self.char_embeddings(char_ids))
        char_hidden = None
        _, (hidden_last, c_last) = self.char_lstm(char_embeds, char_hidden)  # hidden_last.shape = (directional, batch_size*seq_length, char_hidden_dim)
        return hidden_last.transpose(1, 0).contiguous().view(batch_size, seq_length, self.char_hidden_dim)